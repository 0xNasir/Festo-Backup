...
// set document information
    ...
    $pdf->SetTitle('Festo - Quotation - Revised - ' . $_POST['quotaNo']);
...
EOD;
    ...
    $pdf->SetFont('times', 'B', 11);
    $postData = 'Quotation No.: ' . $_POST['quotaNo'].' (Revised)';
    $dt = <<<EOD
    $postData
EOD;
    $pdf->writeHTMLCell('', '', 15, 44, $dt, '', 0, false, false, 'L', false);
...
// set some text to print
    ...
    $cmInfo
EOD;
    $pdf->writeHTMLCell('', '', 15, 58, $html, '', 0, false, false, 'L', false);
    ...
    $contactInfo
EOD;
    $pdf->writeHTMLCell('', '', 16, 65, $html, '', 0, false, false, 'L', false);
    ...
    $designation
EOD;
    $pdf->writeHTMLCell('', '', 36, 85, $html, '', 0, false, false, 'L', false);
    ...
    $subject
EOD;
    $pdf->writeHTMLCell('', '', 16, 86, $html, '', 0, false, false, 'L', false);
 ...
 <tr nobr="true">
  ...
  <th>TOTAL PRICE (Taka)</th>
    ...
    foreach ($_POST['productList'] as $product) {
        $productInfo = $productInfo . '<tr nobr="true"><td>' . $counter++ . '</td><td>' . $product['productName'] . '<br>Type: ' . $product['productType'] . '</td><td>' . $product['internalPartNumber'] . '</td><td>' . $product['productQty'] . '</td><td>' . $product['productPrice'] . '</td><td>' . $product['productPrice'] * $product['productQty'] . '</td></tr>';
    ...
    if (isset($_POST['doc']['pdfVat'])) {
        if ($_POST['doc']['pdfVat'] != 0) {
            ...
            $productInfo = $productInfo . '<tr nobr="true"><td colspan="5">VAT ' . $_POST['doc']['pdfVat'] . '%</td><td>' . $vatAmount . '</td></tr>';
...
EOD;
    ...
    foreach ($_POST['terms'] as $term) {
        if ($term['topic']==' '){
            ...
            $terms = '<table style="padding-left: 2px;"><tr><td style="width: 150px;"><strong>' . $term['topic'] . '</strong></td><td style="width: auto;">: ' . $term['message'] . '</td></tr></table><br>';
            $pdf->SetFont('times', '', 10);
            $html = <<<EOD
    $terms
EOD;
...
EOD;
    ...
    $pdf->SetFont('times', 'B', 11);
    $suggestion = "For SINCOS AUTOMATION TECHNOLOGIES LTD.";
    $html = <<<EOD
    <br><br>
    $suggestion
    <br><br>
...
//    $pdf->writeHTML($tls, true, true, true, true, "L");
    $pdf->writeHTMLCell('', '', 16, -40, $tls, 0, 0, false, false, 'L', false);
//Close and output PDF document
    $pdf->Output('Festo_Quotation_Revised_' . $_POST['quotaNo'] . '.pdf', 'I');
