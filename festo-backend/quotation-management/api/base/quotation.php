...
httpRESTMethod::post(function ($data){
    ...
    global $db;
    $rst=$db->query("INSERT INTO `quotation` (`quota_id`, `quota_date`, `quota_no`, `quota_ref`, `company_name`,`branch_name`,`address`, `contact_person`,`person_id`,`designation`, `status`, `contact_by`,`contact_by_designation`,`contact_by_phone`, `remarks`, `managed_by`, `state`, `doc_data`, `terms_condition`, `print_count`) VALUES(NULL, '$data->date', '$data->quotaNo','$data->quotaRef', '$data->companyName', '$data->branchName', '$data->branchAddress', '$data->contactPerson', '$data->personId', '$data->designation', '$data->status', '$data->contactBy','$data->contactByDesignation', '$data->contactByPhone', '$data->remarks','$managed_by','Incomplete', '', '', 0)");
    if ($rst){
    ...
    if ($rst){
        ...
        foreach ($product_list as $item){
            ...
            if ($item->productId!=null){
                ...
                $result=$db->query("UPDATE `product` SET `product_name` = '$item->productName', `product_part_no` = '$item->productPartNumber',`internal_part_no` = '$item->internalPartNumber', `product_type` = '$item->productType', `product_quantity` = '$item->productQty', `available_quantity` = '$item->productQtyAvailable', `product_price` = '$item->productPrice', `product_description` = '$item->productDescription' WHERE `product`.`product_id` ='$item->productId'");
            }else{
                $rdt["entry"]=true;
                if ($item->productPrice=='0.00'){
                    $hasEmptyPrice=true;
                }
                $result=$db->query("INSERT INTO `product` (`product_id`, `quotation_id`, `product_name`, `product_part_no`, `internal_part_no`, `product_type`, `product_quantity`, `available_quantity` , `product_price`, `product_description`) VALUES (NULL, '$id', '$item->productName', '$item->productPartNumber', '$item->internalPartNumber', '$item->productType', '$item->productQty','$item->productQtyAvailable', '$item->productPrice', '$item->productDescription')");
                if ($result){
