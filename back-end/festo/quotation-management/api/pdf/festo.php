 ...
 <tr nobr="true">
  ...
  <th>TOTAL PRICE (Taka)</th>
    ...
    foreach ($_POST['productList'] as $product) {
        $productInfo=$productInfo.'<tr nobr="true"><td>'.$counter++.'</td><td style="text-align: left;">'.$product['productName'].'<br>Type: '.$product['productType'].'</td><td>'.$product['internalPartNumber'].'</td><td>'.$product['productQty'].$product['productUnit'].'</td><td>'.$product['productQtyAvailable'].$product['productUnit'].'</td><td style="text-align: right;">'.formatedPrice($product['productPrice']).'</td><td style="text-align: right;">'.formatedPrice(sprintf('%0.2f',$product['productPrice']*$product['productQty'])).'</td></tr>';
...
EOD;
    ...
    // Set font
    $pdf->SetFont('times', 'B', 11);
    $image="http://127.0.0.1/swassertive/appfile/image/public/festo/signature/signature_of_".$_POST['contactByUsername'].".png";
    $signature=<<<EOD
    <img width="150" height="20" style="margin: auto 0 0 0;" src="$image" alt="Signature"/>
EOD;
    $pdf->writeHTML($signature, true, true, true, true, "L");
    // Set font
    $pdf->SetFont('times', 'B', 11);
